...
function LayerCard({ layer, active }) {
  ...
  return (
    <div className={`rounded-xl border transition-all ${active ? "border-orange-400 dark:border-orange-600 bg-orange-50/50 dark:bg-orange-950/20" : "border-gray-100 dark:border-[#1f1f1f] bg-white dark:bg-[#111]"}`}>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-3 p-3 text-left">
        <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${active ? "bg-orange-500 text-white" : "bg-gray-100 dark:bg-[#1f1f1f] text-gray-500 dark:text-gray-400"}`}>
          {layer.id}
        </div>
        <span className={`text-sm font-semibold flex-1 ${active ? "text-orange-600 dark:text-orange-400" : "text-gray-800 dark:text-gray-200"}`}>{layer.name}</span>
    ...
    <PageLayout title="Pipeline Monitor">
      <div className="space-y-5">
        ...
        <div className="insight-card">
          <p className="insight-label mb-1">AEGIS-FIN 9-Layer Pipeline</p>
          <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
            Every day after NSE market close, the scheduler fetches live data for all {" "}
            <span className="font-semibold text-orange-500">134 SME companies</span> from Yahoo Finance,
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { icon: Database,      label: "Total Runs",         value: total,           color: "text-orange-500" },
            { icon: CheckCircle,   label: "Successful",         value: ok,              color: "text-emerald-500" },
            { icon: XCircle,       label: "Failed",             value: errors,          color: "text-red-500" },
            { icon: Activity,      label: "Companies Processed",value: uniqueCompanies, color: "text-blue-500" },
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs text-gray-400">
            <Clock size={13} />
            Last run: <span className="font-semibold text-gray-600 dark:text-gray-300">{lastRun}</span>
          </div>
          <button onClick={load} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-orange-500 border border-orange-200 dark:border-orange-800 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-950/30 transition-colors">
          ...
          <div className="card p-4 sm:p-5">
            ...
            <ResponsiveContainer width="100%" height={140}>
              <BarChart data={chartData}>
                ...
                <Tooltip {...ct.tooltip} />
                <Bar dataKey="ok"    stackId="a" fill={ct.emerald} radius={[0,0,0,0]} name="OK" />
                <Bar dataKey="error" stackId="a" fill={ct.red}     radius={[3,3,0,0]} name="Error" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
                ...
                {logs.map(row => (
                  <div key={row.id}>
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold text-gray-800 dark:text-gray-200 flex-1 truncate">{row.company}</span>
                    ...
                    {expanded === row.id && (
                      <div className="mx-2 mb-1 p-3 bg-gray-50 dark:bg-[#1a1a1a] rounded-xl border border-gray-100 dark:border-[#2a2a2a]">
                        {row.error_msg && (
        ...
        <div className="card p-4 sm:p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              ...
              <div key={title} className="p-4 bg-gray-50 dark:bg-[#1a1a1a] rounded-xl border border-gray-100 dark:border-[#2a2a2a]">
                <p className="text-xs font-bold text-gray-800 dark:text-gray-200 mb-2">{title}</p>
                <code className="block text-xs font-mono text-orange-500 bg-orange-50 dark:bg-orange-950/30 px-2 py-1.5 rounded-lg mb-2">{cmd}</code>
                <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
          <div className="mt-3 p-3 bg-amber-50 dark:bg-amber-950/20 rounded-xl border border-amber-200 dark:border-amber-900">
