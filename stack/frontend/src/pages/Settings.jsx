...
export default function Settings() {
  ...
  if (!currentUser) return <PageLayout><div>Loading settings...</div></PageLayout>;

  const { appSettings } = currentUser;

  const toggleEmailAlerts = () => {
    setCurrentUser(prev => ({
      ...prev,
      appSettings: { ...prev.appSettings, emailAlerts: !prev.appSettings.emailAlerts }
    }));
  };
  ...
  const toggleTheme = () => {
    setCurrentUser(prev => ({
      ...prev,
      appSettings: { ...prev.appSettings, theme: prev.appSettings.theme === 'dark' ? 'light' : 'dark' }
    }));
  };
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-[1fr,150px] gap-2 sm:gap-6 items-start sm:items-center">
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                    <input type="number" value={appSettings.volatilityRatio} onChange={(e) => updateSetting('volatilityRatio', Number(e.target.value))} step="0.05" className="w-full bg-transparent font-bold text-right text-emerald-700 font-mono outline-none" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[1fr,150px] gap-2 sm:gap-6 items-start sm:items-center">
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                    ...
                    <input type="number" value={appSettings.sentimentZScore} onChange={(e) => updateSetting('sentimentZScore', Number(e.target.value))} step="0.1" className="w-full bg-transparent font-bold text-right text-rose-500 font-mono outline-none" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[1fr,150px] gap-2 sm:gap-6 items-start sm:items-center">
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                    <input type="number" value={appSettings.debtEquityRatio} onChange={(e) => updateSetting('debtEquityRatio', Number(e.target.value))} step="0.1" className="w-full bg-transparent font-bold text-right text-gray-700 font-mono outline-none" />
              ...
              <div className="pt-8 border-t border-gray-100">
                <div className="flex justify-between items-center bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                  <div>
                    ...
                    <p className="text-xs text-emerald-700 mt-1">Receive daily digests and instant spike alerts to {currentUser.email}</p>
                  </div>
                  <button onClick={toggleEmailAlerts}>
                    {appSettings.emailAlerts ? <ToggleRight size={40} className="text-[#2d6a4f]" /> : <ToggleLeft size={40} className="text-gray-300" />}
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={toggleTheme}>
                    {appSettings.theme === 'dark' ? <ToggleRight size={40} className="text-[#2d6a4f]" /> : <ToggleLeft size={40} className="text-gray-300" />}
                  </button>
                </div>
                ...
                <div>
                  ...
                  <select value={appSettings.defaultSector} onChange={(e) => updateSetting('defaultSector', e.target.value)} className="w-64 bg-gray-50 border border-gray-200 rounded-lg p-2 font-bold text-emerald-700 outline-none">
                ...
                <div>
                  ...
                  <select value={appSettings.currency} onChange={(e) => updateSetting('currency', e.target.value)} className="w-64 bg-gray-50 border border-gray-200 rounded-lg p-2 font-bold text-emerald-700 outline-none">
