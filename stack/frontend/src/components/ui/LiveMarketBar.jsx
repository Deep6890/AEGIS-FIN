...
export default function LiveMarketBar() {
  ...
  return (
    <div className="card px-4 py-3 flex items-center gap-3 overflow-hidden">
      {/* Live indicator */}
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-[10px] font-semibold uppercase tracking-widest text-[var(--text-3)] hidden sm:block">Live</span>
      ...
      <div className="flex items-center gap-2 overflow-x-auto flex-1" style={{ scrollbarWidth: "none" }}>
        {data.map(({ symbol, label, price, change }) => (
          <div key={symbol} className="flex items-center gap-2 shrink-0 px-3 py-1.5 rounded-xl bg-neutral-50 dark:bg-neutral-900/60 border border-[var(--border)]">
            <span className="text-[11px] font-medium text-[var(--text-2)] whitespace-nowrap">{label}</span>
            {price != null ? (
              <>
                ...
                <span className={`flex items-center gap-0.5 text-[10px] font-bold tabular-nums ${change >= 0 ? "text-[var(--orange)]" : "text-[var(--text-3)]"}`}>
